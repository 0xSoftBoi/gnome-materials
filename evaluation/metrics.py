import matplotlib
matplotlib.use('Agg')  # Use headless backend to avoid display issues
import matplotlib.pyplot as plt
import numpy as np


def plot_comparison(histories, strategy_names, output_path='results/comparison.png',
                    top_k=10, initial_train=50, k_per_iter=20):
    ...
    Args:
        ...
        top_k: number of top structures tracked (10 for synthetic, 100 for CHGNet)
        initial_train: initial labeled set size (for sample efficiency x-axis)
        k_per_iter: candidates selected per iteration (for sample efficiency x-axis)
    """
    ...
    # Plot 2: Top-k discovery efficiency
    ax = axes[1]
    for i, (history, name) in enumerate(zip(histories, strategy_names)):
        top10 = history['top10_efficiency']
        ax.plot(range(1, len(top10) + 1), top10, marker='s', label=name, color=colors[i], linewidth=2)
    ax.set_xlabel('Iteration')
    ax.set_ylabel(f'Top-{top_k} Discovered (Fraction)')
    ax.set_title(f'Top-{top_k} Discovery Efficiency')
    ax.set_ylim([0, 1.05])
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def print_summary(histories, strategy_names, top_k=10):
    ...
    for history, name in zip(histories, strategy_names):
        ...
        print(f"  Top-{top_k} discovered: {top10_final:.1%}")
